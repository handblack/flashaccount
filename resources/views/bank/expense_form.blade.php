            ...
            <div class="row">
                ...
                <div class="col-md-3 console" style="line-height: 1; border-left:1px solid #dcdcdc;">
                    <dl class="row mb-0" >
                        ...
                        <dd class="col-sm-7">{{ $row->bankaccount->shortname }}</dd>
                ...
                <div class="col-md-3 console" style="line-height: 1; border-left:1px solid #dcdcdc;">
                    <dl class="row mb-0">
                        <dt class="col-sm-5">Egreso</dt>
                        <dd class="col-sm-7 text-right"><span class="total-sum-pay">{{ number_format($row->amount,env('DECIMAL_AMOUNT',2)) }}</span></dd>
                        ...
                        <dd class="col-sm-7 text-right"><span class="total-sum-anticipo">{{ number_format($row->amount,env('DECIMAL_AMOUNT',2)) }}</span></dd>
                    </dl>
                </div>
            </div>
        </div>
            ...
            <table class="table table-hover text-nowrap table-sm  mb-0">
                <thead>
                    <tr style="background-color:#dcdcdc;">
                        ...
                        <th class="text-right pr-2">Abierto {{ $row->currency->currencyiso }}</th>
                        <th class="text-right mr-2">Aplicar {{ $row->currency->currencyiso }}</th>
                ...
                <tbody>
                    ...
                    @forelse ($open as $line)
                        <tr id="{{ $line->token }}">
                            ...
                            <td width="140" class="align-middle">
                                {{ $line->doctype->doctypecode }}-{{ $line->serial }}-{{ $line->documentno }}
                            </td>
                            ...
                            <td width="120">
                                <input type="text" name="apply[]" data-id="{{ $line->id }}" id="input-apply-{{ $line->token }}" class="text-right form-control form-control-sm apply-sum" value=""  required disabled>
                            </td>
                    ...
                    {{-- ANTICIPOS --}}
                    @foreach ($ant as $line)
                        <tr id="{{ $line->token.'-anticipo' }}">
                            <td class="align-middle" width="60">
                                <div class="custom-control custom-checkbox">
                                    <input class="custom-control-input checkBoxAnticipo"name="cha[]" type="checkbox" id="customCheckboxAnticipo{{ $line->id }}" data-token="{{ $line->token.'-anticipo' }}" value="{{ $line->id }}">
                                    <label for="customCheckboxAnticipo{{ $line->id }}" class="custom-control-label"></label>
                                </div>
                            </td>
                            <td width="90" class="align-middle">{{ $line->datetrx }}</td>
                            <td width="140" class="align-middle">
                                ANTICIPO
                            </td>
                            <td width="120" class="text-right align-middle pr-2"  style="border-right:1px solid #dcdcdc;">
                            ...
                            <td width="120">
                                <input type="text" name="applya[]" data-id="{{ $line->id }}" id="input-apply-{{ $line->token.'-anticipo' }}" class="text-right form-control form-control-sm apply-sum" value="" required disabled>
                            </td>
                            <td class="text-right align-middle">                                
                ...
                @if(!$ant->isEmpty())
                    <tfoot>
                        <tr>
                            <td colspan="4"></td>
                            <td class="text-right font-weight-bold pr-2">{{ number_format($line->sum('amount'),env('DECIMAL_AMOUNT',2)) }}</td>
...
$(function(){
    ...
    $('.checkBoxInvoice,.checkBoxAnticipo').change(function() {
